...
const QuizApp = () => {
                ...
                await axios.get('https://opentdb.com/api.php?amount=5&category=9&difficulty=medium&type=multiple').then(response => {
                    ...
                    result.map(data => {
                        ...
                        for (i = 0; i < data.incorrect_answers.length; i++) {
                            options[i] = {
                                option: data.incorrect_answers[i],
                                isSelected: false
                            }
                        }
                        options[i] = {
    ...
    return (
        <div className={'d-flex justify-center align-items-center flex-direc-col ' + QuizStyles.quizBox}>
            ...
            {isLoading && <Loading />}
            {quizStart ? quizData.length > 0 ? <QuizQuestions data={quizData} checkAnswers={checkAnswers} /> : <QuizStart quizStart={setQuizStart} /> : quizReset ? <QuizAnswer quizReset={setQuizReset} data={quizData} /> : <QuizStart quizStart={setQuizStart} />}
